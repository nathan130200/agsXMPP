...
namespace AgsXMPP.Xml.Xpnet
{
	...
	public abstract class Encoding
	{
		...
		private TOK scanDecl(byte[] buf, int off, int end, Token token)
		{
			...
			{
				case BT_MINUS:
					return this.scanComment(buf, off + this.minBPC, end, token);
				case BT_LSQB:
					token.TokenEnd = off + this.minBPC;
			...
			while (off != end)
			{
				switch (this.byteType(buf, off))
				{
					case BT_PERCNT:
						...
						switch (this.byteType(buf, off + this.minBPC))
						{
							case BT_S:
							case BT_CR:
							case BT_LF:
		...
		{
			var upper = false;
			if (end - off != this.minBPC * 3)
				return false;
			switch (this.byteToAscii(buf, off))
			{
				case 'x':
					break;
				case 'X':
		...
		{
			/* "CDATA[".length() == 6 */
			if (end - off < 6 * this.minBPC)
				throw new PartialTokenException();
			for (var i = 0; i < CDATA.Length; i++, off += this.minBPC)
				this.checkCharMatches(buf, off, CDATA[i]);
			token.TokenEnd = off;
										...
										Token token)
		{
			...
			switch (this.byteType(buf, off))
			{
				...
				case BT_CR:
					...
					if (this.byteType(buf, off) == BT_LF)
						off += this.minBPC;
					token.TokenEnd = off;
		...
		private TOK scanEndTag(byte[] buf, int off, int end, Token token)
		{
			...
			while (off != end)
			{
				switch (this.byteType(buf, off))
				{
					...
					case BT_LF:
						token.NameEnd = off;
						for (off += this.minBPC; off != end; off += this.minBPC)
						{
							switch (this.byteType(buf, off))
							{
								case BT_S:
		...
		private TOK scanHexCharRef(byte[] buf, int off, int end, Token token)
		{
			if (off != end)
			{
				int c = this.byteToAscii(buf, off);
				int num;
				switch (c)
				{
					case '0':
		...
		private TOK scanCharRef(byte[] buf, int off, int end, Token token)
		{
			if (off != end)
			{
				...
				{
					case 'x':
						return this.scanHexCharRef(buf, off + this.minBPC, end, token);
					case '0':
					case '1':
					case '2':
					case '3':
		...
		private bool isMagicEntityRef(byte[] buf, int off, int end, Token token)
		{
			...
			{
				case 'a':
					if (end - off < this.minBPC * 4)
						break;
					switch (this.byteToAscii(buf, off + this.minBPC))
					{
						case 'm':
							if (this.charMatches(buf, off + this.minBPC * 2, 'p')
								&& this.charMatches(buf, off + this.minBPC * 3, ';'))
							{
								token.TokenEnd = off + this.minBPC * 4;
								token.RefChar1 = '&';
								return true;
							}
							break;
						case 'p':
							if (end - off >= this.minBPC * 5
								&& this.charMatches(buf, off + this.minBPC * 2, 'o')
								&& this.charMatches(buf, off + this.minBPC * 3, 's')
								&& this.charMatches(buf, off + this.minBPC * 4, ';'))
							{
								token.TokenEnd = off + this.minBPC * 5;
								token.RefChar1 = '\'';
								return true;
							}
							break;
				...
				case 'l':
					if (end - off >= this.minBPC * 3
						&& this.charMatches(buf, off + this.minBPC, 't')
						&& this.charMatches(buf, off + this.minBPC * 2, ';'))
					{
						token.TokenEnd = off + this.minBPC * 3;
						token.RefChar1 = '<';
						return true;
					}
					break;
				...
				case 'q':
					if (end - off >= this.minBPC * 5
						&& this.charMatches(buf, off + this.minBPC, 'u')
						&& this.charMatches(buf, off + this.minBPC * 2, 'o')
						&& this.charMatches(buf, off + this.minBPC * 3, 't')
						&& this.charMatches(buf, off + this.minBPC * 4, ';'))
					{
						token.TokenEnd = off + this.minBPC * 5;
						token.RefChar1 = '"';
		...
		private TOK scanRef(byte[] buf, int off, int end, Token token)
		{
			...
			switch (this.byteType(buf, off))
			{
				...
				case BT_LEAD4:
					...
					break;
				case BT_NUM:
					return this.scanCharRef(buf, off + this.minBPC, end, token);
				default:
					throw new InvalidTokenException(off);
			}
			while (off != end)
				...
				case BT_EXCL:
					...
					switch (this.byteType(buf, off))
					{
						case BT_MINUS:
							return this.scanComment(buf, off + this.minBPC, end, token);
						case BT_LSQB:
							return this.scanCdataSection(buf, off + this.minBPC, end, token);
					}
					throw new InvalidTokenException(off);
				case BT_QUEST:
					return this.scanPi(buf, off + this.minBPC, end, token);
				case BT_SOL:
					return this.scanEndTag(buf, off + this.minBPC, end, token);
				default:
					throw new InvalidTokenException(off);
			}
			/* we have a start-tag */
				...
				switch (this.byteType(buf, off))
				{
					...
					case BT_LF:
						...
						for (; ; )
						{
							...
							{
								case BT_NMSTRT:
									return this.scanAtts(off, buf, off + this.minBPC, end, token);
								case BT_LEAD2:
									if (end - off < 2)
										throw new PartialCharException(off);
									if (this.byteType2(buf, off) != BT_NMSTRT)
								   ...
								   ContentToken token)
		{
			...
			{
				case BT_LT:
					return this.scanLt(buf, off + this.minBPC, end, token);
				case BT_AMP:
					return this.scanRef(buf, off + this.minBPC, end, token);
				case BT_CR:
					off += this.minBPC;
					if (off == end)
						throw new ExtensibleTokenException(TOK.DATA_NEWLINE);
			...
			token.TokenEnd = this.extendData(buf, off, end);
			return TOK.DATA_CHARS;
		}
		...
		public TOK tokenizeProlog(byte[] buf, int off, int end, Token token)
		{
			...
			{
				case BT_QUOT:
					return this.scanLit(BT_QUOT, buf, off + this.minBPC, end, token);
				case BT_APOS:
				...
				case BT_LF:
					...
					return TOK.PROLOG_S;
				case BT_PERCNT:
					return this.scanPercent(buf, off + this.minBPC, end, token);
				case BT_COMMA:
					token.TokenEnd = off + this.minBPC;
					return TOK.COMMA;
				case BT_LSQB:
				...
				case BT_RSQB:
					...
					if (this.charMatches(buf, off, ']'))
					{
						if (off + this.minBPC == end)
							throw new PartialTokenException();
						if (this.charMatches(buf, off + this.minBPC, '>'))
						{
							token.TokenEnd = off + 2 * this.minBPC;
				...
				case BT_NUM:
					return this.scanPoundName(buf, off + this.minBPC, end, token);
				case BT_LEAD2:
					if (end - off < 2)
						throw new PartialCharException(off);
					switch (this.byteType2(buf, off))
					{
						case BT_NMSTRT:
							off += 2;
							tok = TOK.NAME;
			...
			while (off != end)
			{
				switch (this.byteType(buf, off))
				{
					...
					case BT_RSQB:
						...
						{
							if (level == 0)
								return off + this.minBPC;
							level--;
						}
						else if (this.charMatches(buf, off, ']'))
							break;
						off += this.minBPC;
						break;
					default:
		...
		public string getPublicId(byte[] buf, int off, int end)
		{
			...
			for (; off != end; off += this.minBPC)
			{
				var c = this.byteToAscii(buf, off);
				switch (this.byteType(buf, off))
				{
					case BT_MINUS:
					case BT_APOS:
		...
		{
			var len = str.Length;
			if (len * this.minBPC != end - off)
				return false;
			for (var i = 0; i < len; off += this.minBPC, i++)
			{
				if (!this.charMatches(buf, off, str[i]))
					return false;
			}
			return true;
		}
