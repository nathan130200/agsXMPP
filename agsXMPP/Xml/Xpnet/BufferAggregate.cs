...
namespace agsXMPP.Xml.Xpnet
{
	...
	{ // RingBuffer of the Nieblung
		...
		public void Write(byte[] buf)
		{
			...
			if (this.m_tail == null)
			{
				this.m_head = this.m_tail = new BufNode();
				this.m_head.buf = buf;
			}
			else
			{
		...
		public void Clear(int offset)
		{
			...
			BufNode bn = null;
			for (bn = this.m_head; bn != null; bn = bn.next)
			{
			...
			if (save > 0)
			{
				...
				System.Buffer.BlockCopy(this.m_head.buf,
										this.m_head.buf.Length - save,
										buf, 0, save);
				this.m_head.buf = buf;
			}
			...
			for (bn = this.m_head; bn != null; bn = bn.next)
			{
				this.m_stream.Write(bn.buf, 0, bn.buf.Length);
			}
		}
