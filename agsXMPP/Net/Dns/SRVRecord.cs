...
namespace AgsXMPP.Net.Dns
{
	...
	public class SRVRecord : RecordBase, IComparable
	{
		...
		{
			this.m_Priority = pointer.ReadShort();
			this.m_Weight = pointer.ReadShort();
			this.m_Port = pointer.ReadShort();
			this.m_Target = pointer.ReadDomain();
		}
		...
		public int CompareTo(object obj)
		{
			...
			if (this.m_Priority < srvOther.m_Priority) return -1;
			if (this.m_Priority > srvOther.m_Priority) return 1;
			...
			if (this.m_Weight > srvOther.m_Weight) return -1;
			if (this.m_Weight < srvOther.m_Weight) return 1;
